//
//  NewIJKFFMoviePlayerController.m
//  IJKMediaPlayer
//
...
#import "NewIJKFFMoviePlayerController.h"
...
@interface NewIJKFFMoviePlayerController() <ASIHTTPRequestDelegate>
...
@implementation NewIJKFFMoviePlayerController {
    IJKFFMrl *_ffMrl;
               ...
               withOptions:(IJKFFOptions *)options
{
    self = [super init];
    if (self) {
        NSString* urlHeader = [NSString stringWithUTF8String:"http://cloudapi.hupu.com/live/httpcdn?token="];
        NSString* urlString = [urlHeader stringByAppendingString:token];
        
        ASIFormDataRequest *formDataRequest = [ASIFormDataRequest requestWithURL:nil];
        NSString* encodedUrlString = [formDataRequest encodeURL:urlString];
        tokenUrl = [NSURL URLWithString:encodedUrlString];

        //http request
        ASIHTTPRequest *request = [ ASIHTTPRequest requestWithURL :tokenUrl];
...
- (void)shutdownWaitStop:(NewIJKFFMoviePlayerController *) mySelf
{
    if (!_mediaPlayer)
        return;
...
- (void)shutdownClose:(NewIJKFFMoviePlayerController *) mySelf
{
    if (!_mediaPlayer)
        return;
...
inline static NewIJKFFMoviePlayerController *ffplayerRetain(void *arg) {
    return (__bridge_transfer NewIJKFFMoviePlayerController *) arg;
}
...
int media_player_msg_loop(void* arg)
{
    @autoreleasepool {
        ...
        __weak NewIJKFFMoviePlayerController *ffpController = ffplayerRetain(ijkmp_set_weak_thiz(mp, NULL));
...
int onControlResolveSegment(NewIJKFFMoviePlayerController *mpc, int type, void *data, size_t data_size)
{
    if (mpc == nil)
        return -1;
...
{
    NewIJKFFMoviePlayerController *mpc = (__bridge NewIJKFFMoviePlayerController*)opaque;
...
- (void)audioSessionInterrupt:(NSNotification *)notification
{
    ...
    switch (reason) {
        case AVAudioSessionInterruptionTypeBegan: {
            NSLog(@"NewIJKFFMoviePlayerController:audioSessionInterrupt: begin\n");
            [self pause];
            [[IJKAudioKit sharedInstance] setActive:NO];
            break;
        }
        case AVAudioSessionInterruptionTypeEnded: {
            NSLog(@"NewIJKFFMoviePlayerController:audioSessionInterrupt: end\n");
